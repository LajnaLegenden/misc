...
export class AStar {
  process(grid, startNode, endNode, options) {
 ...
 */
    ...
    while (openList.length > 0) {
      var currentIndex = this.getNextIndex(openList);//asd
      ...
      var neighbors = this.getNeighbors(currentNode, grid, options.diagonal);
      for (var i = 0; i < neighbors.length; i++) {
        var neighbor = neighbors[i];

        if (neighbor.closed || neighbor.block != "air") {
          continue;
        }
  ...
  heuristic = function (nodeA, nodeB, heuristic, heightFactor) {
  ...
  distance = function (nodeA, nodeB, heuristic, heightFactor) {
    if (heuristic == "manhatten") {
      return (
        Math.abs(nodeA.x - nodeB.x) +
        Math.abs(nodeA.y - nodeB.y) +
        Math.abs(nodeA.z - nodeB.z) * heightFactor
      );
    } else if (heuristic == "euclidean") {
      return Math.sqrt(
        Math.pow(nodeA.x - nodeB.x, 2) +
          Math.pow(nodeA.y - nodeB.y, 2) +
          Math.pow(nodeA.z - nodeB.z, 2) * heightFactor
      );
    }
  };

  getNextNode = function (openList) {
    var nextNode;
    var minimumF;
  ...
  getNextIndex = function (openList) {
    var nextIndex = 0;
    var minimumF;
    for (var i = 0; i < openList.length; i++) {
  ...
  getNeighbors = function (aNode, node, diagonal) {
    var neighbors = [];
    var x = aNode.x;
    var y = aNode.y;
    var z = aNode.z;
    ...
    for (let i = 0; i < blocks.length; i++){
      let obj = blocks[i];
      let item = _.get(node, obj.l, new Block({}, obj.cord, true))
      _.set(node, obj.l, item);
      neighbors.push(item)
    }
    //4-Way
    /* 
