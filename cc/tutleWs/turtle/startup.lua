...
if string_match(tostring(1 / 2), "%p") == "," then
    ...
    function convertreal(v) return string_gsub(_convertreal(v), ',', '.') end
end
...
local function decode_unicode_surrogate(s1, s2)
    return utf8_char(0x10000 + (tonumber(s1, 16) - 0xd800) * 0x400 +
                         (tonumber(s2, 16) - 0xdc00))
end

local function decode_unicode_escape(s) return utf8_char(tonumber(s, 16)) end
    ...
    while true do
        ...
        if nx == 117 --[[ "u" ]] then
            ...
            if not decode_escape_set[nx] then
                statusPos = i
                decode_error("invalid escape char '" ..
...
local function decode_number()
    local num, c = string_match(statusBuf, '^([0-9]+%.?[0-9]*)([eE]?)',
                                statusPos)
...
local function decode_number_zero()
    ...
    if not num or string_byte(num, -1) == 0x2E --[[ "." ]] or
        string_match(statusBuf, '^.[0-9]+', statusPos) then
        decode_error("invalid number '" .. get_word() .. "'")
    end
    if c ~= '' then
        num = string_match(statusBuf, '^([^eE]*[eE][-+]?[0-9]+)[ \t\r\n%]},]',
...
local function decode_object()
    statusPos = statusPos + 1
    local res = {}
    if consume_byte "^[ \t\r\n]*}" then return setmetatable(res, json.object) end
    statusTop = statusTop + 1
...
local function unexpected_character()
    decode_error("unexpected character '" ..
                     string_sub(statusBuf, statusPos, statusPos) .. "'")
end
local function unexpected_eol() decode_error("unexpected character '<eol>'") end
...
function json.decode(str)
    ...
    statusTop = 0
    local res = decode()
    while statusTop > 0 do decode_item() end
...
function undergoMitosis()
    ...
    if not turtle.place() then return nil end
    ...
    if not turtle.suck() then return nil end
...
function websocketLoop()
    ...
    local ws, err = http.websocket("ws://proxy3001.linusjansson.com")

    if err then
        print(err)
    elseif ws then
        local cordsHandle = io.open("cords");
        local cords = cordsHandle.read(cordsHandle)
        if fs.exists("/disk/cords") then
            cordsHandle = io.open("/disk/cords");
            cords = cordsHandle.read(cordsHandle)
        end
        while true do
            term.clear()
            ...
            if obj.type == 'cords' then
                ws.send(json.encode({cords = cords, nonce = obj.nonce}))
            end
            if obj.type == 'saveCords' then
                fs.delete('/cords');
                cords = fs.open("cords", "w")
                cords.write(obj.cords)
                cords.close()
                cordsHandle = io.open("cords");
                cords = cordsHandle.read(cordsHandle)
                ws.send(json.encode({data = cords, nonce = obj.nonce}))
            end
        end
