...
public:
	...
	vector<Block> process(Block start, Block end)
	{
		...
		while (openList.size() > 0 && traversedNodes < 20000)
		{
			...
			{
				cout << traversedNodes << " - "
					 << "X: " << currentBlock.x << " Y: " << currentBlock.y << " Z: " << currentBlock.z << endl;
			}
...
private:
	...
	vector<Block> getNeighbours(Block self)
	{
		...
		{
			neighbours.push_back(Block(world[to_string(x)][to_string(y)][to_string(z + 1)],x,y,z+1));
		}
		return neighbours;
	}
};
...
int main(int argc, char **argv)
{
	...
	Block start(world["517"]["152"]["530"],517,152,530);
